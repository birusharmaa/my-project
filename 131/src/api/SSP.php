    ...
    static function filter ( $request, $columns, &$bindings, $isJoin = false )
    {
        ...
        if ( isset($request['search']) && $request['search']['value'] != '' ) {
            ...
            for ( $i=0, $ien=count($request['columns']) ; $i<$ien ; $i++ ) {
                ...
                if ( $requestColumn['searchable'] == 'true' ) {
                    ...
                    $search_column =  explode(" as",$column['db']);
                    $search_column = $search_column[0];
                    $globalSearch[] = ($isJoin) ? $search_column." LIKE ".$binding : "`".$search_column."` LIKE ".$binding;
                }
            }
        }
        ...
        for ( $i=0, $ien=count($request['columns']) ; $i<$ien ; $i++ ) {
            ...
            if ( $requestColumn['searchable'] == 'true' &&
                ...
                $search_column =  explode(" as",$column['db']);
                    $search_column = $search_column[0];
                $columnSearch[] = ($isJoin) ? $search_column." LIKE ".$binding : "`".$search_column."` LIKE ".$binding;
            }
        }
